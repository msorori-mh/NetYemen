...
class MySupportScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('دعمي'), actions: [
          if (staff)
            IconButton(
                tooltip: 'قائمة الدعم',
        ...
        floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (_) => const NewSupportCaseScreen())),
        ...
        body: RefreshIndicator(
            ...
            child: items.when(
                ...
                error: (e, _) => _State(
                    icon: Icons.error_outline, text: 'تعذر تحميل التذاكر\n$e'),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
        ...
        body: items.when(
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (e, _) => _State(
                icon: Icons.error_outline, text: 'تعذر تحميل القائمة\n$e'),
            data: (all) {
              ...
              return open.isEmpty
                  ? const _State(
                      icon: Icons.inbox_outlined, text: 'لا توجد حالات معلقة')
                  : ListView.builder(
                      padding: const EdgeInsets.all(12),
                      itemCount: open.length,
                      itemBuilder: (_, i) =>
                          CaseTile(item: open[i], agent: true));
            }));
  }
}
...
class _NewSupportCaseState extends ConsumerState<NewSupportCaseScreen> {
  ...
  Widget _field(TextEditingController c, String label, int min,
          {int lines = 1, bool optional = false}) =>
      TextFormField(
          controller: c,
          maxLines: lines,
...
class SupportCaseScreen extends ConsumerWidget {
  ...
  const SupportCaseScreen(
      {super.key, required this.caseId, this.agentView = false});
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
        ...
        body: item.when(
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (e, _) => _State(
                icon: Icons.error_outline, text: 'تعذر تحميل الحالة\n$e'),
            data: (c) => ListView(padding: const EdgeInsets.all(16), children: [
                  Text('#${c.number} — ${c.subject}',
                      ...
                      data: (m) => Column(
                          children: m
                              .map((x) => ListTile(
                                  ...
                                  title: Text(x.body),
                                  subtitle:
                                      Text(x.createdAt.toLocal().toString())))
                              .toList())),
                  if (c.status != 'closed')
                    FilledButton.icon(
                        onPressed: () => _textAction(
  ...
  Future<void> _textAction(BuildContext context, WidgetRef ref, String label,
      Future<void> Function(String) fn) async {
  ...
  Future<void> _workflow(
      BuildContext context, WidgetRef ref, SupportCase c) async {
                    ...
                    content: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      if (c.type == SupportCaseType.dispute)
                        DropdownButtonFormField<String>(
                            ...
                            decoration:
                                const InputDecoration(labelText: 'النتيجة'),
                            items: [
                              'answered',
                              'fixed',
                              'not_reproducible',
    ...
    if (ok == true) {
      await _act(
          context,
          ref,
          () => ref.read(supportRepositoryProvider).updateStatus(caseId, status,
              resolution: resolution.text, outcome: outcome));
    }
    resolution.dispose();
  }
}
