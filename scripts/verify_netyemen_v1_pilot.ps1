...
$expectedTests = 1..14 | ForEach-Object { '{0:D3}' -f $_ }
...
if (Compare-Object $expectedTests $actualTests) {
    throw "SQL suite numbering must be unique and contiguous 001..014: $($actualTests -join ', ')"
}
...
SELECT CASE WHEN
  ...
  (SELECT binding_status FROM public.notification_transport_config WHERE id=1) = 'unbound' AND
